...
function pulse_get_completion_state($course, $cm, $userid, $type, $pulse = null, $completion = null, $modinfo = null) {
    ...
    $status = $type;
    // Module availability completion for student.
    if ($pulse->completionavailable) {
        // Check the module is visible to the user or not.
        $usermodinfo = get_fast_modinfo($course->id, $userid);
        $usercm = $usermodinfo->get_cm($cm->id);
        $isvisble = $usercm->uservisible;
        if ($isvisble) {
...
function mod_pulse_cm_info_view(cm_info $cm) {
    ...
    // Update the completion status to unknown if the module is available and user is student.
    if (!empty($pulse->completionavailable) && \mod_pulse\helper::pulse_user_isstudent($cm->id)) {
        $completioninfo = new \completion_info($course);
        if ($completioninfo->is_enabled($cm)) {
            $current = $completioninfo->get_data($cm, false, $USER->id);
...
function mod_pulse_inplace_editable($itemtype, $itemid, $newvalue) {
    ...
    if ($itemtype === 'templatetitle') {
        ...
        return new \core\output\inplace_editable(
            ...
            $record->title,
            get_string('edittemplatetitle', 'pulse'),
    ...
    if ($iscurrentuser) {
        ...
        if (has_capability('mod/pulse:viewtemplateslist', $systemcontext)) {
            ...
            $pulsenode = new core_user\output\myprofile\node(
                'pulse',
                'pulse',
                get_string('pulsetemplink', 'mod_pulse'),
                null,
                $automationtemplate
            );
...
function mod_pulse_output_fragment_get_confirmation_content(array $params) {
    ...
    $contenthtml = format_text($contenthtml, $pulse->completionbtn_contentformat);
    return $contenthtml;
}
