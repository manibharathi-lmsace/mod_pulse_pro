...
class mod_pulse_mod_form extends moodleform_mod {
    ...
    public function add_completion_rules() {
        ...
        $mform->addElement('checkbox', 'completionwhenavailable' . $suffix, get_string('completewhenavailable', 'pulse'));
        $mform->setDefault('completionwhenavailable'  . $suffix, 0);
        $mform->addHelpButton('completionwhenavailable'  . $suffix, 'completewhenavailable', 'mod_pulse');
    ...
    public function validation($data, $files) {
        ...
        $suffix = $this->get_suffix();
        if (!empty($data['completionapproval' . $suffix]) && empty($data['completionapprovalroles' . $suffix])) {
            $errors['completionrequireapproval' . $suffix] = get_string('completionapprovalrolesrequired', 'pulse');
        }
    ...
    public function definition_completionbuttonoption(&$mform) {
        ...
        $btncontenthtml = format_text($btncontenthtml, FORMAT_HTML);
