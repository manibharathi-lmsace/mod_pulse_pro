...
$string['activitybackupnotset'] = 'No backup template file found for this preset. Please edit the preset and upload a valid backup file.';
...
$string['approvedsuccess'] = 'Approval successfully';
...
$string['completewhenavailable'] = 'Completion when available';
$string['completewhenavailable_help'] = 'If enabled, the activity will be considered completed when the user has access to it (i.e. when it is available based on availability restricitions).';
...
$string['completionapprovalrolesrequired'] = 'Please select at least one role for approval completion.';
...
###### Key features
*  **Notifications** – Each Pulse activity can be configured to send a notification once it becomes available to the student. Notifications can be personalised with placeholders (e.g. student name, course name) and sent to students, teachers, non-editing teachers, managers, or other course roles.
*  **Completion workflows** – Pulse supports activity completion in three ways (in addition to Moodle core): upon availability, when marked complete by the student, and when approved by another role.

###### Common issues
If Pulse does not work as expected, here are the most common causes:

*  The course has not **started** – Pulse only works in a currently running course. Check that the course start date is in the past.
*  The course has **ended** – Check that the course end date is either not set or is in the future.
*  The course has **enrolled students** – Pulse (free) only works for enrolled students. Check that the course has at least one user with an active student enrolment.
*  The Pulse activity is not **available** – Pulse only triggers when the activity is visible to the student. If unsure, log in as the student: if they can see the activity it is enabled; if not, it is disabled.
*  The **cron job** is not running – Credits and notifications are awarded via a scheduled task. Ensure the cron job runs regularly (Moodle recommends every minute).';
$string['modulename_link'] = 'Pulse';
$string['modulename_summary'] = 'Pulse sends notifications to students when course activities become available to them, supporting engagement and compliance workflows. It provides flexible completion criteria including self-marking, role-based approval, and availability-triggered completion, plus preset templates for fast setup.';
...
<p><strong>Candidate set.</strong> Confirmed, non-deleted, non-suspended users excluding the guest account. For a course-level instance, also requires an active enrolment in that course, unless an enabled condition opts out of the enrolment gate (e.g. Not enrolled). The query is capped at a maximum (default 5,000 users); a trailing "+" on the count means the cap was reached.</p>
...
<li><strong>Enrolment:</strong> EXISTS an active enrolment in the instance&rsquo;s course. When <em>Upcoming</em> is on, also requires the enrolment&rsquo;s <code>timecreated &ge;</code> the upcoming time.</li>
...
<p><strong>What it does not simulate:</strong> notification delay, frequency limits, and suppression rules. Those run at send time; this preview only tells you who currently matches the trigger conditions (and, for course-level instances, the active-enrolment gate).</p>';
...
$string['privacy:metadata:pulseaction'] = 'The Pulse module uses action subplugins (pulseaction) which may store their own user data. See each action subplugin for details.';
$string['privacy:metadata:pulseaddon'] = 'The Pulse module uses addon subplugins (pulseaddon) which may store their own user data. See each addon subplugin for details.';
$string['privacy:metadata:pulsecompletion'] = 'Pulse user activity completions';
$string['privacy:metadata:pulsecondition'] = 'The Pulse module uses condition subplugins (pulsecondition) which may store their own user data. See each condition subplugin for details.';
...
$string['restrictionmet'] = 'Completion when available: met';
