...
class auto_templates extends table_sql {
    ...
    public function col_title($row) {
        ...
        foreach ($actions as $key => $action) {
            if (!$action->is_templateaction_enabled($row->id)) {
                continue;
            }
        ...
        return implode(' ', array_column($values, 'icon')) . $OUTPUT->render($title) . implode(
            ' ',
            array_column($values, 'badge')
        );
    }
    ...
    public function col_actions($row) {
        ...
        [$totalcount, $disabledcount] = $this->get_instance_count($row);
