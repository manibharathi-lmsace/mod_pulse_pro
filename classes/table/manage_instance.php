...
class manage_instance extends \table_sql implements dynamic_table {
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        $categoriesjson = $DB->get_field('pulse_autotemplates', 'categories', ['id' => $this->templateid]);
        $categories = $categoriesjson ? json_decode($categoriesjson) : [];
        if (!empty($categories)) {
            [$categoryinsql, $categoryinparams] = $DB->get_in_or_equal($categories, SQL_PARAMS_NAMED, 'cate');
        ...
        if ($this->filterset->has_filter('category')) {
            ...
            if (!empty($category)) {
                [$insql, $cateinparams] = $DB->get_in_or_equal($category, SQL_PARAMS_NAMED, 'cat');
        ...
        if ($this->filterset->has_filter('course')) {
            ...
            if (!empty($course)) {
                [$insql, $courseinparams] = $DB->get_in_or_equal($course, SQL_PARAMS_NAMED, 'co');
