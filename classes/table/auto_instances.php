...
class auto_instances extends table_sql {
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        $params = [];
        $condition = 'ai.courseid=:courseid ';
    ...
    public function col_title($row) {
        ...
        $title = new \core\output\inplace_editable(
            ...
            $row->title,
            'Edit template title',
            'New value for ' . format_string($row->title)
        );
        ...
        foreach ($actions as $key => $action) {
            if (!$action->is_instanceaction_enabled($row->id)) {
                continue;
            }

            $classname = 'pulseaction_' . $action->get_component();
            $result['badge'] = \html_writer::tag(
                'span',
                get_string('formtab', 'pulseaction_' . $action->get_component()),
                ['class' => 'badge badge-primary ' . $classname]
            );
            $result['icon'] = \html_writer::span($action->get_action_icon(), 'action', ['class' => 'action-icon ' . $classname]);
        ...
        return $collapseicon . implode(' ', array_column($values, 'icon')) . $OUTPUT->render($title) .
            implode(' ', array_column($values, 'badge'));
    }
    ...
    public function col_idnumber($row) {
        $title = html_writer::tag(
            'h5',
            format_string($row->reference . ($row->idnumber ?? '')),
    ...
    public function col_actions($row) {
        ...
        if ($row->status == \mod_pulse\automation\templates::STATUS_ORPHANED) {
            ...
            $reporturl = new \moodle_url($listurl, ['action' => 'report']);
            // Other action plugins report view.
            instances::include_actions_reportsview($actions, $row, $reporturl);
