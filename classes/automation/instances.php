...
class instances extends templates {
    ...
    public function get_instance_formdata() {
        ...
        $this->formdatacache = (array) $instance;
        return $this->formdatacache;
    }
    ...
    public function include_conditions_data(&$instance) {
        ...
        // Backward compatibility, if the condition data is in string format then decode it to array.
        // Fix - PLS-1003.
        if (is_string($conditiondata)) {
            $conditiondata = json_decode($conditiondata, true);
        }
    ...
    public function update_status(bool $status, bool $instance = false) {
        ...
        // PLS-1014: Reset upcomingtime on re-enable so completions during the disabled period don't trigger upcoming conditions.
        if ($status == self::STATUS_ENABLE) {
            $DB->set_field_select(
                'pulse_condition_overrides',
                'upcomingtime',
                time(),
                'instanceid = :instanceid AND status = :status',
                [
                    'instanceid' => $this->instanceid,
                    'status' => condition_base::FUTURE,
                ]
            );
        }
    ...
    public function duplicate() {
        ...
        $instance->status = self::STATUS_DISABLE;
    ...
    public function get_report_url() {
        ...
        foreach ($actionplugins as $component => $pluginbase) {
            if ($pluginbase->get_component() === $report) {
                ...
                $reportid = self::get_reportid($reportsource, $component, $this->instanceid);
     ...
     * @param int $instanceid The automation instance ID (0 = unfiltered).
     *
     * @return int
     */
    public static function get_reportid($reportsource, $component, $instanceid = 0) {
        global $DB;
    ...
    /**
     * Course-level cleanup for all action plugins.
     *
     * @param int $courseid
     * @return void
     */
    public static function delete_course_actions(int $courseid): void {
        $actionplugins = \mod_pulse\plugininfo\pulseaction::get_list();
        foreach ($actionplugins as $name => $plugin) {
            $plugin->delete_course_action($courseid);
        }
    }
    ...
    public function get_user_enrolment_createtime($userid, $course) {
        ...
        if ($context == null || $courseid != $course->id) {
        ...
        if (!empty($enrolments)) {
            $latest = \mod_pulse\helper::get_latest_user_enrolment($enrolments);
            return $latest->timecreated;
        }
    ...
    public static function manage_instance($formdata) {
        ...
        // For existing instances, get current status to preserve it when not overridden.
        $currentstatus = null;
        if (isset($formdata->instanceid) && $formdata->instanceid) {
            $existinginstance = $DB->get_record('pulse_autoinstances', ['id' => $formdata->instanceid], 'status');
            $currentstatus = $existinginstance ? $existinginstance->status : null;
        }
        ...
        $instancedata = (object) [
            ...
            'status' => $formdata->status ?? $currentstatus ?? $templatedata->status,
        ];
