...
abstract class action_base {
    ...
    public function get_tablename() {
        ...
        $table = (new \xmldb_table('pulseaction_' . $this->component))->getName();
        return $table;
    }
    ...
    /**
     * Deletes an action instance's data.
     *
     * @param int $instanceid The ID of the instance.
     * @return bool True.
     */
    ...
    /**
     * Checks if the action is enabled for a specific template.
     * @param int $templateid The ID of the template.
     * @return bool True if enabled, false otherwise.
     */
    public function is_templateaction_enabled(int $templateid): bool {
        global $DB;
        ...
        $table = 'pulseaction_' . $this->component;
        if ($record = $DB->get_record($table, ['templateid' => $templateid], 'id, actionstatus')) {
            return $record->actionstatus == self::ACTION_ENABLED;
        }
    ...
    /**
     * Checks if the action is enabled for a specific instance.
     *
     * @param int $instanceid The ID of the instance.
     * @return bool True if enabled, false otherwise.
     */
    public function is_instanceaction_enabled(int $instanceid): bool {
        global $DB;
        ...
        $table = 'pulseaction_' . $this->component . '_ins';
        $status = $DB->get_field($table, 'actionstatus', ['instanceid' => $instanceid], IGNORE_MISSING);

        if (!is_null($status)) {
            return $status == self::ACTION_ENABLED;
        }

        return $this->is_templateaction_enabled(
            $DB->get_field('pulse_autoinstances', 'templateid', ['id' => $instanceid], MUST_EXIST)
        );
    }
    ...
    public function include_data_forinstance(&$instance, $addprefix = true) {
        ...
        foreach ($actioninstancedata as $configname => $configvalue) {
            // Count the overrides.
            if (
                $configvalue !== null && array_key_exists($configname, (array) $actiondata)
                && $configname != 'id' && $configname != 'dynamiccontent'
                && !in_array($prefix . "_" . $configname, $this->default_override_elements())
            ) {
        ...
        $actiondata['mod'] = !empty($actiondata['dynamiccontent'])
            ? get_coursemodule_from_id('', $actiondata['dynamiccontent']) : [];
    ...
    /**
     * Get email placeholders for this action.
     *
     * @return array An array of email placeholders.
     */
    ...
    /**
     * Validate the template form data.
     *
     * @param array $data The form data.
     * @param array $files The form files.
     * @param object $mform The Moodle form object.
     * @param mixed $instance The form instance.
