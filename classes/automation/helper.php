...
class helper {
    ...
    public static function get_templates_forinstance($courseid = null) {
        ...
        $sql = "SELECT * FROM {pulse_autotemplates}
            WHERE (categories = '[]' OR categories = '' OR categories IS NULL OR $like) AND visible = 1";
    ...
    public static function get_addtemplate_instance($form, $courseid) {
        ...
        if (has_capability('mod/pulse:viewtemplateslist', $context)) {
            ...
            $html .= \html_writer::link(
                $manageurl->out(true),
                get_string('managetemplate', 'pulse'),
    ...
    public static function get_templates_tablehelps() {
        ...
        array_walk($actions, function (&$value) {
            ...
            $result['badge'] = \html_writer::tag(
                'span',
                get_string('formtab', 'pulseaction_' . $value->get_component()),
     ...
     */
    public function timetable_installed() {
        global $CFG;
        static $result;
     ...
     * @return string
     */
    public function timetable_details(string $var, \stdClass $course, int $userid, $context = null, $mod = null): string {
        global $CFG, $DB, $PAGE;
        ...
        if ($var == 'eventdates') {
            ...
            [$data, $template] = calendar_get_view($calendar, 'upcoming_mini');
        ...
        if (
            $mod && $var == 'activityduedate' &&
            ($record = $DB->get_record('tool_timetable_modules', ['cmid' => $mod->cmid ?? 0]))
        ) {
            if ($record) {
                $timemanagement = new \tool_timetable\time_management($record->course);
                $userenrolments = $timemanagement->get_course_user_enrollment($userid, $record->course);
                ...
                $duedate = !empty($moduledates) ? $moduledates['duedate'] : 0;
                return isset($duedate) ? userdate($duedate) : '';
            }

            return '';
        }

        if ($mod && $var == 'upcomingmods') {
            $mods = $this->tool_timetable_get_upcoming_activities($course->id, $userid);
            return implode(', ', array_map(
                fn($mod) => \html_writer::link(
                    (new \moodle_url("/mod/$mod->modname/view.php", ['id' => $mod->id]))->out(false),
    ...
    /**
     * Get upcoming activities list from tool timetable.
     *
     * @param int $courseid
     * @param int $userid
     * @return string
     */
    protected function tool_timetable_get_upcoming_activities($courseid, $userid) {
        global $DB;
        $upcomingmods = [];
        $modinfo = get_fast_modinfo($courseid);
        $timemanagement = new \tool_timetable\time_management($courseid);
        $userenrolments = $timemanagement->get_course_user_enrollment($userid, $courseid);
                ...
                if (!empty($modnumbers)) {
                    foreach ($modnumbers as $modnumber) {
                        ...
                        if (
                            ...
                            $moduledata = $timemanagement->prepare_enhanced_module_data($mod, $userid);
                            if (!empty($moduledata)) {
                                if (!isset($moduledata['duedatetimestamp']) || $moduledata['duedatetimestamp'] <= time()) {
                                    continue;
                                }
                                $upcomingmods[$moduledata['cmid']] = $mod;
                            }
                        }
                    }
                }
    ...
    public static function get_filter_button($filtered) {
        ...
        $filter = new \manage_instance_table_filter(
            $url->out(false) . '#manage-instance-tab',
            ['id' => $templateid, 'numberofinstance' => null]
        );
