...
class automation_instance_form extends automation_template_form {
    ...
    protected function add_override_element($element, $course) {
        ...
        $overrideelement = $mform->createElement(
            'advcheckbox',
            $name,
            '',
            '',
            ['group' => 'automation', 'class' => 'custom-control-input'],
            [0, 1]
        );

        // Insert the override checkbox before the element.
        if (
            isset($mform->_elementIndex[$orgelementname]) && $mform->_elementIndex[$orgelementname]
