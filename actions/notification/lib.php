...
function pulseaction_notification_output_fragment_preview_instance_content($args) {
    ...
    if (isset($args['instanceid'])) {
        ...
        $scheduleuser = (object) ($args['userid'] != null ? core_user::get_user($args['userid']) : current($users));
        $relateduser = (object) !empty($args['relateduserid']) && $args['relateduserid'] > 0
            ? core_user::get_user($args['relateduserid']) : $scheduleuser;
        ...
        if ($sessionincondition && $formdata->condition['session']['status']) {
            ...
            schedule::instance()->include_session_data($mod, $sessionconditiondata, $relateduser->id);
        }
...
function pulseaction_notification_output_fragment_preview_content($args) {
    ...
    if (isset($args['contentheader'])) {
        ...
        [$subject, $messagehtml] = mod_pulse\helper::update_emailvars($content, '', $course, $user, $mod, $sender);
        // User selector.
