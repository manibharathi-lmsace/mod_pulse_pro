...
class external extends \external_api {
    ...
    public static function get_chapters_parameters() {
        return new \external_function_parameters(
            ['mod' => new \external_value(PARAM_INT, 'Book module cmid ', VALUE_DEFAULT)]
        );
    }
    ...
    public static function get_chapters($mod = null) {
        ...
        $params = self::validate_parameters(self::get_chapters_parameters(), ['mod' => $mod]);

        if (isset($params['mod'])) {
            $cmid = $params['mod'];
            $context = \context_module::instance($cmid);
            self::validate_context($context);
            require_capability('mod/pulse:addinstance', $context->get_course_context());
