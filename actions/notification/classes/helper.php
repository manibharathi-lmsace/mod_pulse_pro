...
class helper {
     ...
     * @param  mixed $msghtml Message in HTML format.
     * @param  mixed $pulse
     * @param  mixed $sender
     ...
     * @return void
     */
    public static function messagetouser($userto, $subject, $messageplain, $msghtml, $pulse, $sender = true, $cc = [], $bcc = []) {
        global $CFG;
        ...
        $eventdata->fullmessagehtml = $msghtml;
    ...
    public static function message_send(\core\message\message $eventdata) {
        ...
        if (!$eventdata->notification && !$eventdata->convid) {
            ...
            if (
                !isset($eventdata->userto->auth) || !isset($eventdata->userto->suspended)
            ...
            if ($eventdata->userfrom->id == $eventdata->userto->id) {
                ...
                if (
                    !$conversationid = \core_message\api::get_conversation_between_users([$eventdata->userfrom->id,
                    $eventdata->userto->id ])
                ) {
