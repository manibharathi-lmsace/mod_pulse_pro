...
Feature: coursedates trigger event.
  ...
  Scenario: Course dates Upcoming with Start date already passed does not trigger
    And I set course "C1" start date to "-2" days
    ...
    # Instance Conditions - Course dates as Upcoming, start date already in the past.
    And I click on ".action-edit" "css_element" in the "Template1" "table_row"
    Then I follow "Condition"
    And I click on "#id_override_triggeroperator" "css_element" in the "#pulse-condition-tab" "css_element"
    Then the field "Trigger operator" matches value "All"
    ...
    # A start date that already passed before the condition was enabled must NOT trigger.
    And I click on "#notification-action-report" "css_element" in the "Template1" "table_row"
    And I switch to a second window
    And I should see "Nothing to display" in the ".reportbuilder-wrapper" "css_element"
    And I close all opened windows

  Scenario: Course dates Upcoming with Start date in the future triggers
    And I set course "C1" start date to "2" days
    And I navigate to course "Course 1" automation instances
    And I wait until the page is ready
    And I open the autocomplete suggestions list
    ...
    # Instance Conditions - Course dates as Upcoming, start date still in the future.
    And I click on ".action-edit" "css_element" in the "Template1" "table_row"
    Then I follow "Condition"
    And I click on "#id_override_triggeroperator" "css_element" in the "#pulse-condition-tab" "css_element"
    Then the field "Trigger operator" matches value "All"
    ...
    # A start date still in the future when the condition was enabled must trigger.
    And I click on "#notification-action-report" "css_element" in the "Template1" "table_row"
    And I switch to a second window
    And the following should exist in the "reportbuilder-table" table:
      | Course full name | Message type | Subject                   | Full name      | Time created          | Scheduled time        | Status |
      | Course 1         | Template1    | Course date notification  | student User 1 | ##now##%A, %d %B %Y## | ##today +2days##%A, %d %B %Y## | Queued |
    And I close all opened windows

  Scenario: Course dates Upcoming with End date already passed does not trigger
    And I set course "C1" end date to "-2" days
    And I navigate to course "Course 1" automation instances
    And I wait until the page is ready
    And I open the autocomplete suggestions list
    And I click on "Template1" item in the autocomplete list
    ...
    # Instance Conditions - Course dates as Upcoming, end date already in the past.
    And I click on ".action-edit" "css_element" in the "Template1" "table_row"
    Then I follow "Condition"
    And I click on "#id_override_triggeroperator" "css_element" in the "#pulse-condition-tab" "css_element"
    Then the field "Trigger operator" matches value "All"
    ...
    # An end date that already passed before the condition was enabled must NOT trigger.
    And I click on "#notification-action-report" "css_element" in the "Template1" "table_row"
    And I switch to a second window
    And I should see "Nothing to display" in the ".reportbuilder-wrapper" "css_element"
    And I close all opened windows
