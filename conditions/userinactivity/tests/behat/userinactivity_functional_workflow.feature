...
Feature: User inactivity trigger condition - Functional workflow tests
  ...
  Scenario: User inactivity in Upcoming status only flags users after the condition was enabled
    # Create automation template
    Given I create automation template with the following fields to these values:
      | Title     | Upcoming Inactivity Alert |
      | Reference | upcomingalert             |
    And I log in as "admin"
    # Student1 is already long inactive before the condition is ever turned on.
    Then I change user enrollment time to "-30" minutes for "student1" in "C1"
    Then I set last course access to "-30" minutes for "student1" in "C1"
    And I log out
    # Configure instance
    And I log in as "admin"
    And I am on "Course 1" course homepage
    And I follow "Automation"
    When I open the autocomplete suggestions list
    And I click on "Upcoming Inactivity Alert" item in the autocomplete list
    Then I press "Add automation instance"
    And I set the following fields to these values:
      | insreference | upcominginactive1 |
    # Configure inactivity condition as "Upcoming" - 5 minutes of no course access.
    Then I follow "Condition"
    And I click on "#id_override_condition_userinactivity_status" "css_element"
    And I set the field "condition[userinactivity][status]" to "Upcoming"
    And I click on "#id_override_condition_userinactivity_type" "css_element"
    And I set the field "condition[userinactivity][type]" to "Based on access"
    And I click on "#id_override_condition_userinactivity_inactivityperiod" "css_element"
    And I set the field "condition[userinactivity][inactivityperiod][number]" to "5"
    ...
    And I set the field "Recipients" in the "#pulse-action-notification" "css_element" to "Student"
    And I click on "#id_override_pulsenotification_subject" "css_element"
    And I set the field "pulsenotification_subject" to "Upcoming Inactivity Reminder"
    And I click on "#id_override_pulsenotification_headercontent_editor" "css_element"
    And I set the field "pulsenotification_headercontent_editor[text]" to "Hi {User_firstname}, you haven't accessed the course recently."
    And I press "Save changes"
    # Run the scheduled task immediately - student1 must NOT be flagged yet, since the
    # inactivity clock restarts at the moment the condition was enabled (upcomingtime).
    And I run all adhoc tasks
    And I am on "Course 1" course homepage
    And I follow "Automation"
    And I click on "#notification-action-report" "css_element" in the "Upcoming Inactivity Alert" "table_row"
    And I switch to a second window
    And the following should not exist in the "reportbuilder-table" table:
      | Full name      | Subject                     |
      | Student User 1 | Upcoming Inactivity Reminder |
    And I close all opened windows
    # Backdate the enabled time so the inactivity period has elapsed since the condition
    # was turned on, then re-run the task - student1 should now be flagged.
    And I set upcoming enabled time to "-10" minutes for condition "userinactivity" in instance "upcominginactive1"
    And I run all adhoc tasks
    And I am on "Course 1" course homepage
    And I follow "Automation"
    And I click on "#notification-action-report" "css_element" in the "Upcoming Inactivity Alert" "table_row"
    ...
    And the following should exist in the "reportbuilder-table" table:
      | Full name      | Subject                     | Status |
      | Student User 1 | Upcoming Inactivity Reminder | Queued |
    And I close all opened windows
