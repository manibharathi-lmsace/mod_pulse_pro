...
class conditionform extends \mod_pulse\automation\condition_base {
    ...
    public function is_user_completed($instancedata, int $userid, ?\completion_info $completion = null) {
        ...
        if ($instancedata->condition['course']['upcomingtime'] ?? 0) {
            $upcomingtime = $instancedata->condition['course']['upcomingtime'];
            $params = [
                'userid'    => $userid,
                'course'  => $courseid,
            ];
            $ccompletion = new \completion_completion($params);
            if (!empty($ccompletion->timecompleted) && $ccompletion->timecompleted < $upcomingtime) {
                return false;
            }
        }
    ...
    public static function course_completed($eventdata) {
        ...
        WHERE ai.courseid=:courseid AND (co.status > 0 OR (co.status IS NULL AND ai.templateid IN (
                SELECT c.templateid FROM {pulse_condition} c WHERE c.triggercondition = 'course' AND c.status > 0
            )
        ))";
