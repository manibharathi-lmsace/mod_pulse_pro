...
/**
 * Privacy implementation for pulseaddon_availability
 *
...
/**
 * Privacy provider for the pulseaddon_availability subplugin.
 */
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('pulseaddon_availability', [
            'userid' => 'privacy:metadata:availability:userid',
            'pulseid' => 'privacy:metadata:availability:pulseid',
            'status' => 'privacy:metadata:availability:status',
            'availabletime' => 'privacy:metadata:availability:availabletime',
        ], 'privacy:metadata:availability');
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        $sql = "SELECT ctx.id
                  ...
                  JOIN {pulseaddon_availability} pa ON pa.pulseid = cm.instance
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $records = $DB->get_records('pulseaddon_availability', [
                'pulseid' => $cm->instance,
                'userid' => $userid,
            ]);
            ...
            $data = array_map(function ($record) {
                return [
                    ...
                    'availabletime' => $record->availabletime
                        ? transform::datetime($record->availabletime) : '-',
                ];
            }, $records);

            writer::with_context($context)->export_data(
                [get_string('pluginname', 'pulseaddon_availability')],
                (object) ['availability' => array_values($data)]
            );
        }
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $cm = get_coursemodule_from_id('pulse', $context->instanceid);
            if ($cm) {
                $DB->delete_records('pulseaddon_availability', [
                    'pulseid' => $cm->instance,
                    'userid' => $userid,
                ]);
            }
