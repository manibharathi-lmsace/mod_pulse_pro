...
class preset_list extends \table_sql implements dynamic {
    ...
    public function col_action(\stdclass $row): string {
        ...
        $editlink = html_writer::link(
            ...
            ['class' => 'edit-template action-icon mx-1 p-1 btn btn-link icon-no-margin']
        );
        ...
        $deletelink = $OUTPUT->action_icon($url, new \pix_icon('t/delete', get_string('deletepreset', 'pulse')), $deleteaction);

        return html_writer::div($editlink . $deletelink, 'd-flex align-items-center');
    }
