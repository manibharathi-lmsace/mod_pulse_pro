...
if ($ADMIN->fulltree) {
    ...
    $settings->add(new admin_setting_configtext(
        ...
        get_string('schedulecount', 'pulse'),
        get_string('schedulecountdesc', 'pulse'),
        500,
        PARAM_INT
    ));
...
$automation = new admin_externalpage(
    'pulseautomation',
    get_string('autotemplates', 'pulse', null, true),
    new moodle_url('/mod/pulse/automation/templates/list.php'),
...
foreach (core_plugin_manager::instance()->get_plugins_of_type('pulseaction') as $plugin) {
    // Load all the dashaddon plugins settings pages.
    $plugin->load_settings($ADMIN, 'modpulse', $hassiteconfig);
}

foreach (core_plugin_manager::instance()->get_plugins_of_type('pulsecondition') as $plugin) {
    // Load all the dashaddon plugins settings pages.
    $plugin->load_settings($ADMIN, 'modpulse', $hassiteconfig);
}
