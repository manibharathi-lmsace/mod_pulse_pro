...
if ($action == 'approve' && $userid) {
    ...
    if ($existrecord = $DB->get_record('pulse_completion', ['userid' => $userid, 'pulseid' => $cm->instance])) {
        $existrecord->approvalstatus = 0;
        $existrecord->approveduser = $USER->id;
        $existrecord->timemodified = time();
        $result = $DB->update_record('pulse_completion', $existrecord);
    } else {
        $record = new stdclass();
        $record->userid = $userid;
        $record->pulseid = $cm->instance;
