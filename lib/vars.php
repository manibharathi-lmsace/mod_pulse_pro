...
class pulse_email_vars {
    ...
    /**
     * Return the list of URL-type placeholder names.
     *
     * @return string[]
     */
    public static function url_placeholders(): array {
        return ['siteurl', 'courseurl', 'linkurl', 'Course_Courseurl', 'Site_Siteurl', 'Mod_Url'];
    }
    ...
    public function get_user_enrolment() {
        ...
        if (!empty($enrolments)) {
            $latestinstance = pulsehelper::get_latest_user_enrolment($enrolments);
            ...
            $effectivestart = $latestinstance->timestart ?: $latestinstance->timecreated;
            return (object) [
                'progress' => round($progress) . '%',
                'courseduedate' => $courseduedate,
                'status' => ($latestinstance->status == 1) ? get_string('suspended', 'mod_pulse') : get_string('active'),
                'startdate' => $effectivestart
                    ? userdate($effectivestart, $datefmt) : $emptystartdate,
                'enddate' => $latestinstance->timeend
                    ? userdate($latestinstance->timeend, $datefmt) : $emptyenddate,
            ];
        }
        return (object) ['startdate' => $emptystartdate, 'enddate' => $emptyenddate];
    }
    ...
    public static function schedule_user_profile_fields() {
        ...
        if ($fields === null) {
            ...
            $removefields = [
                ...
                'scheduleuser_imagealt', 'scheduleuser_moodlenetprofile', 'scheduleuser_secret',
            ];
            $fields = array_values(array_filter($fields, fn($field) => !in_array(strtolower($field), $removefields)));
        }
